...
from custom_components.adaptive_pergola import (
    PLATFORMS,
    async_setup_entry,
    async_unload_entry,
)
...
async def test_building_profile_entry_skips_coordinator(hass) -> None:
    ...
    with (
        patch(
            "custom_components.adaptive_pergola.AdaptiveDataUpdateCoordinator"
        ) as mock_coord,
...
async def test_building_profile_entry_unloads_without_platforms(hass) -> None:
    ...
    with (
        ...
        patch("custom_components.adaptive_pergola.async_unload_services"),
    ):
