...
class TestGeometrySchemaLabels:
    ...
    def test_metric_uses_metres(self):
        schema = geometry_louvered_roof_schema(_hass(imperial=False))
    ...
    def test_imperial_uses_inches(self):
        ...
        for key in self._LENGTH_KEYS:
            ...
            assert cfg["step"] == 0.5
        # Range is converted: 30 m footprint max → ~1181 in after round-up.
        assert _selector_for(schema, CONF_LR_FOOTPRINT_X)["max"] >= 1181
...
@pytest.mark.unit
class TestLouveredSlatLabels:
    """Slat dimensions: cm metric, in imperial."""

    def test_metric_uses_cm(self):
        schema = geometry_louvered_roof_schema(_hass(imperial=False))
        cfg = _selector_for(schema, CONF_LR_SLAT_CHORD)
        assert cfg["unit_of_measurement"] == "cm"

    def test_imperial_uses_inches(self):
        schema = geometry_louvered_roof_schema(_hass(imperial=True))
        for key in (CONF_LR_SLAT_CHORD, CONF_LR_SLAT_THICKNESS, CONF_LR_SLAT_SPACING):
            cfg = _selector_for(schema, key)
            assert cfg["unit_of_measurement"] == "in"
            assert cfg["step"] == 0.05
        # 60 cm chord max → ~23.62 in → rounded up at 0.05 step.
        assert _selector_for(schema, CONF_LR_SLAT_CHORD)["max"] >= 23.6
