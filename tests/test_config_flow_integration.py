...
# Create-flow geometry payload. The flow always builds a louvered roof (the
# only shipped cover type), and its geometry schema supplies every field from
# defaults — so an empty submit is valid and keeps these walkthroughs focused
# on routing rather than on restating the roof's dimensions.
_GEOMETRY: dict = {}
...
async def test_create_flow_creates_entry(hass: HomeAssistant) -> None:
    """The lean create flow builds a louvered-roof entry with safe defaults.

    No name/type step: the louvered roof is the only cover type, and with no
    cover picked there is no device to derive a name from, so the fallback
    instance name applies.
    """
    ...
    entry = result["result"]
    assert entry.data[CONF_SENSOR_TYPE] == CoverType.LOUVERED_ROOF
    assert entry.data["name"] == "Pergola"
    # Proxy cover is always shipped by the create flow (no toggle to answer).
    assert entry.options[CONF_ENABLE_PROXY_COVER] is True
...
async def test_options_flow_sync_empty_selection_no_abort(hass: HomeAssistant) -> None:
    ...
    # Condensed flow: multi-cover sync is heritage — not offered in the menu.
    assert result["type"] == "menu"
    assert "sync" not in result.get("menu_options", [])
...
def test_get_azimuth_edges_sums_fov():
    ...
    from custom_components.adaptive_pergola.config_flow import _get_azimuth_edges
    from custom_components.adaptive_pergola.const import CONF_FOV_LEFT, CONF_FOV_RIGHT
...
@pytest.mark.unit
def test_get_geometry_schema_unknown_type_returns_louvered():
    """_get_geometry_schema falls back to the louvered-roof schema for unknown types."""
    ...
    assert result is GEOMETRY_LOUVERED_ROOF_SCHEMA
...
async def test_options_flow_menu_includes_blind_spot_when_enabled(
    ...
    result = await hass.config_entries.options.async_init(entry.entry_id)
    assert result["type"] == "menu"
    # Condensed flow: blind_spot is heritage — never offered, even when the
    # legacy enable flag is set.
    assert "blind_spot" not in result.get("menu_options", [])
...
async def test_options_menu_order_follows_pipeline_layers(
    ...
    # L1 → L2 (condensed: blind_spot/interp/glare/sync/profiles never shown)
    assert idx("cover_entities") < idx("geometry") < idx("sun_tracking")
    assert idx("sun_tracking") < idx("position")
    for heritage in ("blind_spot", "interp", "glare_zones", "sync", "building_profile"):
        assert heritage not in menu, f"heritage step {heritage} back in menu {menu}"
    ...
    assert idx("temperature_climate") < idx("automation")
...
async def test_options_menu_every_entry_has_a_label(
    ...
    from custom_components.adaptive_pergola.const import CONF_INTERP
...
def test_config_flow_does_not_use_system_language() -> None:
    ...
    import custom_components.adaptive_pergola.config_flow as cf_module
...
async def test_options_flow_position_saves_position_tolerance(
    ...
    from custom_components.adaptive_pergola.const import CONF_POSITION_TOLERANCE
    from tests.ha_helpers import VERTICAL_OPTIONS, _patch_coordinator_refresh
...
async def test_config_flow_cover_entities_no_devices_skips_device_selector(
    ...
    with patch(
        "custom_components.adaptive_pergola.config_flow._get_devices_from_entities",
...
async def test_create_flow_title_falls_back_to_adaptive_prefix_without_device(
    ...
    assert result["type"] == "create_entry"
    entry = result["result"]
    assert entry.title == "Louvered Roof Adaptive Living Room Blind"
...
@pytest.mark.integration
async def test_create_flow_persists_derived_fov(
    hass: HomeAssistant,
) -> None:
    """Create flow must persist the sun-window-derived CONF_FOV_* to entry.options.
    ...
    values. The window-reveal half of the original guard (CONF_WINDOW_WIDTH and
    the FOV-from-measurements button) is gone with the vertical-blind form: the
    louvered policy leaves ``supports_fov_compute`` False, so that toggle is not
    in the pergola's schema at all.
    """
    result = await hass.config_entries.flow.async_init(
        DOMAIN, context={"source": "user"}
    )
    ...
    # 135°→225° converts to the canonical azimuth 180, fov 45/45.
    result = await hass.config_entries.flow.async_configure(
        result["flow_id"],
        {
            CONF_SUN_WINDOW_START: 135,
    ...
    assert opts.get(CONF_FOV_RIGHT) == 45
    assert opts.get(CONF_AZIMUTH) == 180
    # The transient sun-window fields must never reach persisted options.
    assert CONF_SUN_WINDOW_START not in opts
    assert CONF_SUN_WINDOW_END not in opts
...
async def test_create_flow_ignores_stale_unknown_type_entry(
    ...
    # Condensed flow: no create menu — the cover form must render (no crash
    # from enumerating the stale entry).
    assert result["type"] == "form"
