...
async def test_get_devices_from_entities_no_entities(mock_hass):
    ...
    with (
        patch("custom_components.adaptive_pergola.config_flow.er") as mock_er,
        patch("custom_components.adaptive_pergola.config_flow.dr") as mock_dr,
    ):
...
def test_device_info_standalone_when_no_device_id(mock_hass):
    ...
    entity = AdaptivePergolaBaseEntity.__new__(AdaptivePergolaBaseEntity)
    ...
    assert info.get("manufacturer") == "B4S71"
...
def test_device_info_merged_when_device_id_set(mock_hass):
    ...
    with patch("custom_components.adaptive_pergola.entity_base.dr") as mock_dr:
        device_reg = MagicMock()
        device_reg.async_get.return_value = device_entry
        mock_dr.async_get.return_value = device_reg
