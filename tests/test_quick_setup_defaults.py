...
def _make_coordinator_with_options(options: dict):
    ...
    config entries) by replicating only the two lines that matter:
        ...
        self.manager = AdaptivePergolaManager(hass, self.manual_duration, logger)
    ...
    coord.manager = AdaptivePergolaManager(hass, coord.manual_duration, logger)
    return coord
...
class TestCoordinatorInitWithNoneOptions:
    ...
    def test_manual_duration_not_none_in_coordinator_init(self):
        ...
        assert coord.manual_duration is not None, (
            ...
            "AdaptivePergolaManager will crash with TypeError: timedelta(**None)"
        )
    ...
    def test_manual_override_manager_initialises_without_crash(self):
        """AdaptivePergolaManager must not crash when duration comes from None options.
        ...
        # Must not raise
        manager = AdaptivePergolaManager(hass, duration, logger)
