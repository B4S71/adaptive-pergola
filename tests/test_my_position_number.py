"""Tests for AdaptivePergolaMyPositionNumber (number platform, issue #409)."""
...
def test_platform_number_in_platforms():
    ...
    from custom_components.adaptive_pergola import PLATFORMS

    assert Platform.NUMBER in PLATFORMS


# ---------------------------------------------------------------------------
# Step 2 — Module importable with AdaptivePergolaMyPositionNumber
# ---------------------------------------------------------------------------


def test_number_module_importable():
    """number.py must export AdaptivePergolaMyPositionNumber."""
    from custom_components.adaptive_pergola import number
    ...
    assert AdaptivePergolaMyPositionNumber is not None
...
async def test_number_entity_created_when_entities_configured():
    """async_setup_entry must yield exactly one AdaptivePergolaMyPositionNumber."""
    from custom_components.adaptive_pergola.number import (
        AdaptivePergolaMyPositionNumber,
        async_setup_entry,
    )
    ...
    assert len(added) == 1
    assert isinstance(added[0], AdaptivePergolaMyPositionNumber)
...
def _make_number_entity():
    """Return a minimal AdaptivePergolaMyPositionNumber without HA infrastructure."""
    from custom_components.adaptive_pergola.const import CONF_ENTITIES
    from custom_components.adaptive_pergola.number import (
        AdaptivePergolaMyPositionNumber,
    )
    ...
    entity = AdaptivePergolaMyPositionNumber.__new__(AdaptivePergolaMyPositionNumber)
    entity.hass = hass
...
def test_number_entity_range_matches_const():
    ...
    from custom_components.adaptive_pergola.const import _RANGE_MY_POSITION
...
async def test_set_native_value_calls_apply_options_patch():
    ...
    with (
        patch(
            "custom_components.adaptive_pergola.number.validate_options_patch"
        ) as mock_validate,
        patch(
            "custom_components.adaptive_pergola.number.apply_options_patch",
