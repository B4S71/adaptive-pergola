...
class TestSwitchEnabledDefault:
    ...
    def test_switch_class_accepts_enabled_default(self):
        """AdaptivePergolaSwitch accepts enabled_default parameter."""
        ...
        sig = inspect.signature(AdaptivePergolaSwitch.__init__)
...
class TestSelectorDomains:
    ...
    def test_lux_entity_numeric_domains_with_device_class(self):
        ...
        from custom_components.adaptive_pergola.config_flow import LIGHT_CLOUD_SCHEMA
...
_SCHEMA_OPTIONAL_KEY_PAIRS = [
    ...
    # CUSTOM_POSITION's constant covers the tilt-augmented variant (upstream:
    # venetian), not the bare module-level schema. No shipped pergola type
    # advertises custom-position tilt, so build the include_tilt variant
    # directly for the guard — the optional-keys constant still lists the
    # tilt keys because ``optional_entities`` applies it to every cover type.
    (
        "CUSTOM_POSITION",
        _config_fields.custom_position_schema(include_tilt=True),
        _CUSTOM_POSITION_OPTIONAL_KEYS,
    ),
]
