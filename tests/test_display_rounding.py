...
class TestCoordinatorStateIntCoercion:
    ...
    def _invoke_state(self, pipeline_position: int, interpolated: float) -> int:
        ...
        with patch(
            "custom_components.adaptive_pergola.coordinator.interpolate_position",
...
class TestSensorDisplayPrecision:
    ...
    def test_target_position_sensor_precision_is_zero(self):
        """AdaptivePergolaSensorEntity (Target Position) displays 0 decimals."""
        inst = object.__new__(AdaptivePergolaSensorEntity)
        assert inst.suggested_display_precision == 0

    def test_sun_position_sensor_precision_is_one(self):
        """AdaptivePergolaSunPositionSensor (Sun Position) displays 1 decimal."""
        inst = object.__new__(AdaptivePergolaSunPositionSensor)
