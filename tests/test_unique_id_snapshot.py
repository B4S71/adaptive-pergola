"""Snapshot test that locks every entity unique_id produced by Adaptive Pergola.
...
#   sensor.py     → entity_base.AdaptivePergolaSensorBase: f"{entry_id}_{suffix}"
#                   (TimeSensor passes sensor_name "Start Sun"/"End Sun" — not key)
#   switch.py     → AdaptivePergolaSwitch: f"{entry_id}_{switch_name}" (display name!)
#   binary_sensor → AdaptivePergolaBinarySensor: f"{entry_id}_{key}"
#                   AdaptivePergolaPositionMismatchSensor: f"{entry_id}_position_mismatch"
#   button.py     → AdaptivePergolaButton: f"{entry_id}_Reset Manual Override"
#
