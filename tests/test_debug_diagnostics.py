...
from custom_components.adaptive_pergola.pipeline.types import DecisionStep
from custom_components.adaptive_pergola.diagnostics.event_buffer import EventBuffer
from custom_components.adaptive_pergola.managers.manual_override import (
...
def _make_manager() -> tuple[AdaptivePergolaManager, EventBuffer]:
    """Return an AdaptivePergolaManager and its EventBuffer."""
    hass = MagicMock()
    logger = MagicMock()
    event_buffer = EventBuffer(maxlen=DEFAULT_DEBUG_EVENT_BUFFER_SIZE)
    mgr = AdaptivePergolaManager(hass, {"hours": 2}, logger, event_buffer=event_buffer)
...
# ---------------------------------------------------------------------------
# AdaptivePergolaManager — ring buffer defaults
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# AdaptivePergolaManager — ring buffer records correct actions
# ---------------------------------------------------------------------------
...
class TestRingBufferEvents:
    ...
    def test_position_unavailable_records_rejection(self):
        ...
        with patch(
            "custom_components.adaptive_pergola.cover_types.base.get_open_close_state",
            return_value=None,
        ):
            mgr.handle_state_change(
                states_data=event,
...
# ---------------------------------------------------------------------------
# AdaptivePergolaManager — resize_event_buffer
# ---------------------------------------------------------------------------
...
class TestCoverCommandServiceSnapshots:
    ...
    def _make_svc(self):
        ...
        from custom_components.adaptive_pergola.managers.grace_period import (
            GracePeriodManager,
        )
...
class TestPipelineRegistryEventBuffer:
    ...
    def _make_registry(self, event_buffer=None):
        from custom_components.adaptive_pergola.pipeline.registry import (
            PipelineRegistry,
        )
        from custom_components.adaptive_pergola.pipeline.handlers.default import (
            DefaultHandler,
        )
