...
from custom_components.adaptive_pergola.const import CONF_ENABLE_SUN_TRACKING
...
def test_sun_tracking_enabled_by_default():
    """SolarHandler is present when the flag is absent (default True)."""
...
def test_sun_tracking_enabled_explicitly():
    """SolarHandler is present when the flag is explicitly True."""
...
def test_sun_tracking_disabled_pipeline_falls_through_to_default():
    ...
    # Sun is valid and would normally trigger SolarHandler, but the toggle
    # removes that handler so control falls through to DefaultHandler.
