"""Pytest fixtures for Adaptive Pergola tests."""
...
# Register the test-only stand-in policies for the cover types removed in the
# pergola split (cover_blind / cover_awning / cover_tilt). Must be imported
# before any test module imports config_flow, so shared-infra tests that
# construct services with those cover-type strings keep working. See the
# module docstring in tests/compat_policies.py.
from . import compat_policies  # noqa: F401  (import-for-side-effect)
...
@pytest.fixture
def neutralize_venetian_delays():
    """No-op stand-in for the upstream venetian delay-zeroing fixture.

    Upstream this zeroed the venetian dual-axis sequencer's real-motor sleep
    delays. The venetian cover type (and its sequencer) was removed in the
    pergola split, but shared-infra test files still opt in via
    ``pytest.mark.usefixtures("neutralize_venetian_delays")`` — keep the name
    resolvable so those suites run unchanged.
    """
    return
