...
from custom_components.adaptive_pergola.state.snapshot import CoverCapabilities
...
class TestPolicyAxesDeclarations:
    ...
    @pytest.mark.unit
    def test_blind_tilt_louvered_dont_treat_open_as_sun_blocked(self):
        for cover_type in ("cover_blind", "cover_tilt", "cover_louvered_roof"):
    ...
    params=[
        ...
        (
            ...
            {"has_set_position": False, "has_set_tilt_position": True},
            {
                ...
                # blind/awning get routed to TILT_AXIS too.
...
class TestReadAxisValue:
    ...
    @pytest.mark.unit
    def test_louvered_reads_current_tilt_position(self):
        # The louvered roof's primary (and only) axis is tilt, so
        # read_axis_value returns the tilt value even when a position
        ...
        result = get_policy("cover_louvered_roof").read_axis_value(
            hass, "cover.pergola", caps
        )
        assert result == 30
...
_PRODUCTION_ROOT = _REPO_ROOT / "custom_components" / "adaptive_pergola"
...
def test_exposes_dual_axis_sensor(cover_type: str, expected: bool) -> None:
    """The dual-axis Target Tilt sensor was venetian-only; no pergola type has it.
...
def test_custom_position_includes_tilt(cover_type: str, expected: bool) -> None:
    """The custom-position tilt sliders were venetian-only; no pergola type has them.
...
def test_drift_reset_option_is_venetian_only(cover_type: str, expected: bool) -> None:
    """The drift-reset threshold (#663) was a venetian-only geometry option.

    The venetian type is gone in the pergola split — the config key must not
    surface in any remaining cover type's geometry schema.
    """
...
class TestLiftTravelMetres:
    ...
    @pytest.mark.unit
    def test_louvered_returns_none(self) -> None:
