...
def _build_control(item: dict) -> ControlConfig:
    ...
    return ControlConfig(
        ...
        shadow_casting_wall=ShadowCastingWallConfig(
            enabled=bool(item.get(CONF_HAS_SHADOW_CASTING_WALL, False)),
            length_m=float(item.get(CONF_SHADOW_CASTING_WALL_LENGTH_M, 0.0)),
            height_m=float(item.get(CONF_SHADOW_CASTING_WALL_HEIGHT_M, 0.0)),
            offset_east_m=float(item.get(CONF_SHADOW_CASTING_WALL_OFFSET_EAST_M, 0.0)),
            offset_north_m=float(item.get(CONF_SHADOW_CASTING_WALL_OFFSET_NORTH_M, 0.0)),
        ),
    )
